...
usersRouter.post(  '/users/:uid/suspend',     authenticate(), authorize('admin', 'super_admin'), container.usersController.suspend);
usersRouter.post(  '/users/:uid/reactivate',  authenticate(), authorize('admin', 'super_admin'), container.usersController.reactivate);
usersRouter.delete('/users/:uid',             authenticate(), authorize('admin', 'super_admin'), container.usersController.delete);
usersRouter.patch('/users/:uid/roles',      authenticate(), authorize('admin', 'g12', 'super_admin'), container.usersController.assignRole);
