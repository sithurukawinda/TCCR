import { RevokeTempReportAccessUseCase } from '../../../src/application/use-cases/RevokeTempReportAccessUseCase';
...
const makeUser = (overrides = {}): User =>
  new User({
    ...
    tempReportAccess: true, ...overrides,
  });
...
describe('RevokeTempReportAccessUseCase', () => {
  let repo:       jest.Mocked<IUserRepository>;
  let authClient: jest.Mocked<FirebaseAuthClient>;
  let useCase:    RevokeTempReportAccessUseCase;

  beforeEach(() => {
    jest.clearAllMocks();
    repo       = makeRepo();
    authClient = makeAuthClient();
    useCase    = new RevokeTempReportAccessUseCase(repo, authClient);
  });

  it('sets tempReportAccess=false on user and updates Firebase custom claims', async () => {
    ...
    expect(result.tempReportAccess).toBe(false);
    expect(repo.update).toHaveBeenCalledWith(expect.objectContaining({ tempReportAccess: false }));
    expect(authClient.setTempReportAccess).toHaveBeenCalledWith('uid-1', false);
  });
  ...
  it('is idempotent — works even when tempReportAccess is already false', async () => {
    repo.findById.mockResolvedValue(makeUser({ tempReportAccess: false }));
    repo.update.mockResolvedValue(undefined);
    authClient.setTempReportAccess.mockResolvedValue(undefined);
