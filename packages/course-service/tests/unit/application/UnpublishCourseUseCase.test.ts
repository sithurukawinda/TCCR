import { UnpublishCourseUseCase } from '../../../src/application/use-cases/UnpublishCourseUseCase';
...
describe('UnpublishCourseUseCase', () => {
  ...
  let useCase: UnpublishCourseUseCase;

  beforeEach(() => {
    jest.clearAllMocks();
    repo    = makeRepo();
    useCase = new UnpublishCourseUseCase(repo);
  });

  it('returns a PUBLISHED course to DRAFT', async () => {
  ...
  it('throws 409 INVALID_STATE when course is ARCHIVED', async () => {
