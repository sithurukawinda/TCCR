...
describe('DeleteSemesterUseCase', () => {
  ...
  beforeEach(() => {
    ...
    bsRepo       = makeBsRepo();
    useCase      = new DeleteSemesterUseCase(courseRepo, semesterRepo, subjectRepo, lessonRepo, bsRepo);
  });

  it('hard-deletes semester, cascades to subjects/lessons, decrements semesterCount', async () => {
    ...
    subjectRepo.deleteBySemesterId.mockResolvedValue(undefined);
    bsRepo.deleteBySemesterId.mockResolvedValue(undefined);
    semesterRepo.hardDelete.mockResolvedValue(undefined);
    ...
    expect(subjectRepo.deleteBySemesterId).toHaveBeenCalledWith('s1');
    expect(bsRepo.deleteBySemesterId).toHaveBeenCalledWith('s1');
    expect(semesterRepo.hardDelete).toHaveBeenCalledWith('s1');
  ...
  it('throws 404 SEMESTER_NOT_FOUND when semester does not exist', async () => {
    ...
    expect(semesterRepo.hardDelete).not.toHaveBeenCalled();
  });
