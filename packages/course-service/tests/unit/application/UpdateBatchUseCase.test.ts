import { UpdateBatchUseCase } from '../../../src/application/use-cases/UpdateBatchUseCase';
...
describe('UpdateBatchUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase   = new UpdateBatchUseCase(batchRepo);
  });

  it('updates name on a batch in any state', async () => {
    batchRepo.findById.mockResolvedValue(makeBatch({ state: 'open' }));
    batchRepo.update.mockResolvedValue(undefined);

    const batch = await useCase.execute('batch-1', { name: 'New Name' });

    expect(batch.name).toBe('New Name');
    expect(batchRepo.update).toHaveBeenCalledWith(batch);
  });

  it('updates dates on a draft batch', async () => {
    batchRepo.findById.mockResolvedValue(makeBatch({ state: 'draft' }));
    batchRepo.update.mockResolvedValue(undefined);

    const batch = await useCase.execute('batch-1', { intakeStart: '2026-07-01T00:00:00.000Z' });

    expect(batch.intakeStart).toBe('2026-07-01T00:00:00.000Z');
    expect(batchRepo.update).toHaveBeenCalledWith(batch);
  });
  ...
  it('throws 404 when batch not found', async () => {
    ...
    await expect(useCase.execute('missing', { name: 'X' })).rejects.toMatchObject({
      status:    404,
  ...
  it('throws 409 when changing dates on an open batch', async () => {
    batchRepo.findById.mockResolvedValue(makeBatch({ state: 'open' }));

    await expect(
      useCase.execute('batch-1', { intakeStart: '2026-07-01T00:00:00.000Z' }),
  ...
  it('throws 409 when changing dates on a closed batch', async () => {
    batchRepo.findById.mockResolvedValue(makeBatch({ state: 'closed' }));

    await expect(
      useCase.execute('batch-1', { scheduledOpenAt: '2026-07-01T00:00:00.000Z' }),
    ).rejects.toMatchObject({
      status:    409,
      errorCode: 'INVALID_STATE',
    });
