...
node scripts/migrations/008-batch-semester-dates.js              # create batch_semesters rows from existing semester openDate/endDate; creates "Default intake" batch for courses with dated semesters but no batch
# node scripts/migrations/008-batch-semester-dates.js --dry-run  # preview without writing
...
| `/api/v1/me/courses` | course-service (V2 — must precede `/api/v1/me`) |
...
- `cell-service` (:3009, V2) â€” full Clean Architecture stack. 23 endpoints for cell group CRUD, ownership transfer, network reports, network members, cell report edit, member management, join request workflow, and cell report filing. **Cell types:** `g12 | care | children | outreach` (required on create; filterable on list). **Cell states:** `active | archived` (filterable on list). Cell report idempotency: the `X-Idempotency-Key` request header value is stored as `clientReqId` on the `cell_reports` Firestore document; a composite index enforces uniqueness and the controller returns the existing report on duplicate submission. **Cell report authorization:** only the owning leader, the G12 leader, or `super_admin` may file a report — plain `admin` is explicitly excluded (`FileReportUseCase` checks `isSuperAdmin || isOwner`; throws 403 `FORBIDDEN` otherwise). Cell report photos can be pre-uploaded via `POST /cells/:id/report-photos` (returns URLs to pass in `photoUrls[]`) or submitted inline with `POST /cells/:id/reports` as `multipart/form-data` â€” both routes share the same multer middleware family (`handleReportPhotos` / `handleFileReport`). **Key cell-service behaviours:** `DELETE /cells/:id` is a **hard delete** (not soft-delete/archive); authorized for the cell leader, G12 leader, admin, or super_admin — archived cells cannot be deleted. `PATCH /cells/:id/reports/:rid` enforces a **24-hour edit window** from `createdAt`; only the original filer or `super_admin` may edit; voided reports are immutable; `clientReqId` is immutable (cannot be changed on edit). `GET /cells/mine` returns all active cells where the authenticated user appears in `memberUids[]` (any role; no pagination — intended for the member dashboard). `GET /cells` is role-scoped: **G12 and leader callers see all active cells** (`active` by default — pass `?state=archived` for archived); members/students see all active cells; admins see all cells across all states. (Note: network endpoints `GET /cells/network/reports`, `GET /cells/network/members`, and `GET /cells/network/summary` remain scoped to the G12 leader's own network.) `GET /cells/network/reports` follows the same G12 scoping rule. `GET /cells/network/members` follows the same scoping rule but returns member rosters grouped by cell — each entry has `cellId`, `cellName`, `cellType`, `area`, `leaderUid`, `memberCount`, and a `members[]` array enriched with live profiles from user-service (`GetNetworkMembersUseCase`). `GET /cells/network/summary?month=YYYY-MM` returns a rich reporting dashboard for the Reports page — includes scope totals (total cells/leaders), summary stats (cells held, reports filed, active leaders, active G12s), attendance aggregates (present, roster, rate, visitors, avg satisfaction), list of unreported cells with leader names, weekly breakdown (W1–W5 report count and attendance), meeting-type breakdown, and per-leader aggregation; accessible to `leader`, `g12`, `admin`, `super_admin`; G12 and admin callers see org-wide data, cell leaders see only their own cell. **Join request approve/reject is restricted to `admin` and `super_admin` only** — cell leaders cannot approve their own cell's join requests (`POST /cells/:id/join-requests/:rid/approve|reject` guards: `authorize('admin', 'super_admin')`). `POST /cells/:id/transfer-ownership` is restricted to `admin` and `super_admin` only — leaders and G12s no longer have access. Admin may transfer the leader and/or G12 role independently; publishes `cell.ownership_transferred` to the outbox with `initiatedByOwner: false` (no auto-demotion — previous owner retains their role unless separately demoted). Cell domain events (join requests, approvals, rejections, reports filed, ownership transfer) are all wired to notify and audit handlers â€” see outbox table below.
...
| `courses/{id}/batches` | course-service | auto UUID — V2; state machine: `draft → open → closed`; fields: `name`, `scheduledOpenAt`, `intakeStart`, `intakeEnd`, `capacity`, `state` |
| `batch_semesters` | course-service | `{batchId}_{semesterId}` — V2; join table pairing a batch with a semester template; fields: `batchId`, `semesterId`, `courseId`, `openDate` (YYYY-MM-DD or null), `endDate` (YYYY-MM-DD or null). Auto-created with null dates when a batch or semester is created; cascade-deleted when either is deleted. A batch cannot be opened (`POST /batches/:id/open`) until every row for that batch has both dates set. |
...
`CreateBatchUseCase` auto-transitions to `OPEN` if `scheduledOpenAt` is in the past at creation time. Date fields (`scheduledOpenAt`, `intakeStart`, `intakeEnd`) cannot be changed once a batch leaves `DRAFT`.

**Batch endpoints:** `GET /courses/:id/batches`, `POST /courses/:id/batches`, `GET /batches/:id`, `PATCH /batches/:id`, `POST /batches/:id/open`, `POST /batches/:id/close`.

**Semester scheduling per batch (V2 — `batch_semesters` collection):** Each batch has a schedule row per semester of the course (`batch_semesters` documents with compound ID `{batchId}_{semesterId}`). These rows are auto-created with null dates whenever a batch or semester is created. Admins set dates via:
- `PUT /courses/:courseId/batches/:batchId/semester-dates` — bulk-set `openDate`/`endDate` for every semester at once (atomic); must include one entry per semester.
- `PATCH /courses/:courseId/batches/:batchId/semester-dates/:semesterId` — update a single semester's dates.

**`POST /batches/:id/open` validation (V2):** Before transitioning to `OPEN`, the use case checks that every `batch_semesters` row for that batch has both `openDate` and `endDate` set. If any are null → `400 BATCH_SEMESTER_DATES_INCOMPLETE` with `details.missingSemesterIds[]`. Set all dates via the bulk PUT first.

**Student course view (V2):** `GET /me/courses/:courseId?batchId=:batchId` returns the course structure with `openDate`, `endDate`, and a server-derived `state` (`upcoming | open | closed | unscheduled`) injected from the specified batch's `batch_semesters` rows. Only the caller's own batch is ever returned — this endpoint prevents students from reading another batch's schedule.
...
The collection is generated by `scripts/build-postman-collection.js` â€” rerun it after adding endpoints. The `smoke-test.js` script covers a subset of 53 endpoints; the Newman run (`node scripts/newman-run.js`) exercises the full collection against the local stack. See `postman/README.md` for full usage guide. See `postman/reports-page-api-guide.md` for the frontend API reference for the TCCR Reports Page (5-call flow: `GET /me`, `GET /users/summary`, `GET /cells`, `GET /cells/network/summary`, `GET /cells/network/reports` â€” includes full response shapes and field-to-UI mapping).
...
- **`.claude/APIdocument/Version_02__API_Reference.md`** — V2 API reference covering role-requests, batches, batch-semester scheduling, cells, analytics, and other V2-only endpoints. Current version: 2.31.0.
