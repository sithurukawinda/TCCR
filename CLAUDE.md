...
| `super_admin` | Full access — inherits `admin` (all admin CRUD) plus all lower roles, via `authorize()`; also covers super-admin-only operations (master management, etc.) |
...
`super_admin` **inherits `admin`** inside `authorize()` — it has full access. The effective-role expansion is: `super_admin → [super_admin, admin, g12, leader, student, member]`. A super_admin therefore passes every `authorize('admin')` route (course CRUD, user management, enrollment approvals, etc.) without that route needing to list `super_admin` explicitly. `master` does **not** inherit `super_admin` — they remain separate authorization tracks. `master` effective expansion in `authorize()` is `[master, g12, leader, student, member]` — super_admin is excluded.

**`mustBeOwnerOrAdmin()` ownership bypass:** `admin` **or** `super_admin` in the principal's `roles[]` triggers the bypass — both pass ownership-guarded routes without being the resource owner.
...
- **Auth-failure client contract & diagnostics:** `authenticate()` logs every failure (via `@shared/logger`, token-free) with the error code, request path, and `x-request-id` — `TOKEN_EXPIRED`/`SESSION_EXPIRED` at `info`, `TOKEN_REVOKED`/`ACCOUNT_DISABLED`/`INVALID_TOKEN` at `warn`. Clients must treat these distinctly: **`TOKEN_EXPIRED`** (Firebase's fixed 1-hour token lifetime) → silently refresh the ID token (`getIdToken(true)`) and **retry once**, NOT log out; **`TOKEN_REVOKED`** (explicit logout), **`SESSION_EXPIRED`** (absolute session cap reached — see below), and **`403 ACCOUNT_DISABLED`** (suspended/locked account, surfaced by `auth/user-disabled` under `checkRevoked=true`) → end the session (hard logout, do NOT refresh-and-retry). Treating *every* 401 as a hard logout is the typical cause of the "auto-logged-out after ~1 hour" symptom.
- **Absolute session cap (`SESSION_EXPIRED`):** `authenticate()` enforces a wall-clock session lifetime using the token's `auth_time` claim (the real sign-in time, which does **not** advance on silent token refresh). Once `now - auth_time` exceeds `SESSION_MAX_AGE_SECONDS` (env var, default **4 hours**), it returns `401 SESSION_EXPIRED` and the client must sign in again. This is distinct from `TOKEN_EXPIRED` — the client must NOT refresh-and-retry. The cap is **skipped** on `allowUnverified` routes (so a user past the cap can still cleanly `POST /auth/logout` / revoke), skipped when `auth_time` is absent (legacy tokens), and disabled per-route via `authenticate({ maxSessionAgeSeconds: 0 })`. The 1-hour Firebase ID-token lifetime itself is a platform constant and cannot be extended on the backend — only the refresh behaviour (client) and this absolute cap (server) are configurable.
- `super_admin` **inherits `admin`** inside `authorize()` — effective expansion is `[super_admin, admin, g12, leader, student, member]`. It has full access and passes any `authorize('admin')` route without being listed explicitly. See the Roles section for full detail.
- Ownership-sensitive routes add `mustBeOwnerOrAdmin()` after `authorize()`. `admin` **or** `super_admin` in `roles[]` bypasses ownership.
...
| 401 | `MISSING_TOKEN` — missing/malformed `Authorization` header; `UNAUTHENTICATED` — `authorize()` called without preceding `authenticate()`; `TOKEN_EXPIRED` / `TOKEN_REVOKED` / `INVALID_TOKEN` — Firebase verification failures; `SESSION_EXPIRED` — absolute session cap (`SESSION_MAX_AGE_SECONDS`, default 4 h) reached |
| 403 | Valid token, wrong role or ownership; `EMAIL_NOT_VERIFIED`; `ACCOUNT_DISABLED` — token valid but the Firebase account is disabled (admin suspension or failed-login lockout, surfaced by `checkRevoked=true`) |
...
# Session policy (all services — read by @shared/auth-middleware)
SESSION_MAX_AGE_SECONDS                 # absolute session cap in seconds; default 14400 (4 h). After this long since sign-in (token auth_time), authenticate() returns 401 SESSION_EXPIRED regardless of token refresh. Set 0 to disable.
